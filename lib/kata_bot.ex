defmodule KataBot do
  ...
  @impl true
  def handle_event(
        {:INTERACTION_CREATE, %Interaction{data: %{name: "create_kata"}} = interaction, _ws_state}
      ) do
  ...
  @impl true
  def handle_event(
        {:INTERACTION_CREATE, %Interaction{data: %{name: "rand_kata"}} = interaction, _ws_state}
      ) do
    case KataBot.Commands.rand_kata() do
      {:ok, kata} ->
        kata_msg = kata |> format_single_kata()
  ...
  @impl true
  def handle_event(
        {:INTERACTION_CREATE, %Interaction{data: %{name: "list_kata"}} = interaction, _ws_state}
      ) do
    case KataBot.Commands.list_kata() do
      {:ok, katas} ->
        kata_msg = katas |> format_kata_list()
      ...
      _ ->
        Api.create_interaction_response(interaction, %{
          type: 4,
          data: %{content: "Unable to fetch list."}
        })
    end
  end
  ...
  def handle_event(
        {:INTERACTION_CREATE,
         %Interaction{data: %{name: "get_kata", options: options}} = interaction, _ws_state}
      ) do
